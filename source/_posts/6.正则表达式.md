...
- **元字符**
在正则表达式中具有特殊含义的字符，如`+`号，代表前面的字符必须至少出现一次（1次或多次），iphone+x，可以匹配 iphonex、iphoneex、iphoneeeeex等。
`*` 号代表字符可以不出现，也可以出现一次或者多次（0次、或1次、或多次）
iphone*x，可以匹配iphonx、iphonex、iphoneeex 等。
...
- exec
查找字符串中匹配正则的字符串，返回一个数组(未匹配到则返回null)，和string的match方法相同。
```
var reg = /a/;
reg.exec('abc');
...
```
- test
测试字符串是否匹配正则，匹配返回true，不匹配返回false。
```
var reg = /a/;
reg.test('abc');
=> true
