...
HTTP/2的主要设计思想都是源自Google的SPDY协议，是互联网工程任务组(IETF)对谷歌提出的SPDY协议进行标准化才有了现在的HTTP/2。

- 单一长链接
  
  在HTTP/2中，客户端向某个域名的服务器请求页面的过程中，只会创建一条TCP连接，即使这个页面有可能包含上百个资源。而HTTP/1.x一般会创建6-8条TCP连接来请求这100多个资源。单一的长连接，避免了创建多个TCP连接的开销，提高了吞吐量。

- 多路复用
  HTTP/1.x在一条TCP连接上，多个请求只能串行执行，而HTTP/2可以一次请求。此外HTTP/2还能指定优先级，可以优先下载重要的资源。

- 头部压缩和二进制格式
  HTTP/1.x一直都是plain text，HTTP/2使用HPACK压缩来压缩头部。

- 服务端推送
  这个功能通常被称作缓存推送，服务端可以主动向客户端推送资源。当然这需要客户端显示的允许服务端提供该功能，即使如此客户端也可以自主选择是否需要中断该推送的流。
